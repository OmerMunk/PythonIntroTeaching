...
def filter_short_names(names):
    return list(filter(lambda name: len(name) < 5, names))
...
def filter_by_department(employees):
    return list(filter(lambda emp: emp['department'] == "Data Science", employees))
...
def filter_outliers(data):
    data_mean = mean(data)
    data_std = stdev(data)
    return list(filter(lambda x: (data_mean - data_std) <= x <= (data_mean + data_std), data))
...
def filter_transactions(transactions, threshold):
    return list(filter(lambda txn: txn['type'] == 'debit' and txn['amount'] > threshold, transactions))
