...
# Solution:
def apply_custom_discount(products, discount):
    return list(map(lambda p: {'name': p['name'], 'price': round(p['price'] * (1 - discount / 100), 2)}, products))
...
def group_and_sum_sales(transactions):
    grouped_sales = reduce(lambda acc, transaction: {**acc, transaction[0]: acc.get(transaction[0], 0) + transaction[1]}, transactions, {})
