...
# Solution:
@app.route('/users/exists/<name>', methods=['GET'])
def check_user_exists(name):
    exists = db.session.query(User.id).filter_by(name=name).first() is not None
...
# Solution:
@app.route('/users/search/<pattern>', methods=['GET'])
def search_users_by_name(pattern):
    users = User.query.filter(User.name.like(f"%{pattern}%")).all()
    if not users:
        abort(404, description="No users found matching the pattern")
    return jsonify([user.to_dict() for user in users])
